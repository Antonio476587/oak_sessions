...
export const encryptCryptoJSAES = async (
  ...
  const concatResult = concatUint8Array(
    new TextEncoder().encode("Salted__"),
    salt,
    new Uint8Array(cipherText),
  );
  // TypeScript type checking issue: the encode function actually accepts Uint8Array
  return be(concatResult as unknown as ArrayBuffer);
};
...
const EVPKDF = async (
  ...
  while (rawKey.byteLength < KEY_SIZE + IV_SIZE) {
    const concatData = concatUint8Array(block, passphrase, salt);
    ...
    const newRawKey = concatUint8Array(rawKey, block);
    rawKey = new Uint8Array(newRawKey);
  }
